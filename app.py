...
# 从环境变量或.env文件加载敏感信息是最佳实践
shop = os.environ.get("SHOP_NAME", "一品滋味")  # 优先从环境变量获取
...
def get_printers():
    ...
    try:
        ...
        app.logger.error("无法获取Linux打印机列表: 'lpstat' 命令未找到。请确认CUPS已正确安装。")
        return []
...
def dispatch_print_job(order_data, printer_name, print_method):
    ...
    app.logger.info(f"分发打印任务：订单ID {order_data.get('order_id')}，方式: {print_method}, 打印机: {printer_name}")

    # 确保 shop_name 存在，供两个打印助手统一使用
    ...
    if success:
        app.logger.info(f"订单 {order_data.get('order_id')} 的打印任务已通过 {actual_print_method} 方式成功分发。")
    else:
        app.logger.error(f"订单 {order_data.get('order_id')} 通过 {actual_print_method} 方式分发打印任务失败。")
    return success
...
def fetch_missing_orders(start_time):
    ...
    while has_next_page:
        ...
        try:
            ...
            data = resp.json()
            if "errors" in data: app.logger.error(f"GraphQL Error: {data['errors']}"); break
...
def poll_orders():
    """轮询获取遗漏订单的任务函数。"""
    with app.app_context():  # 确保在Flask应用上下文中运行，以便访问数据库等
...
def initialize():
    """在应用首次请求前初始化相关功能。"""
    global first_request, scheduler_started
    if first_request:
        init_db()
        polling_enabled = get_setting('polling_enabled') == 'true'
        if polling_enabled and not scheduler.running:
...
def settings():
    ...
    if request.method == "POST":
        ...
        set_setting("auto_print_enabled", str(request.form.get("auto_print_enabled") == 'on').lower())
        set_setting("print_method", request.form.get("print_method"))
        polling_enabled_new = request.form.get("polling_enabled") == 'on'
        set_setting("polling_enabled", str(polling_enabled_new).lower())
...
def verify_webhook_signature(request):
    ...
    if not md5_received or not ALLVALUE_WEBHOOK_SECRET:
        app.logger.warning("Webhook 验证失败：缺少必要的 Header 或 Secret。")
        return False
    ...
    if md5_calculated != md5_received:
        app.logger.warning(f"Webhook 签名无效。Calculated: {md5_calculated}, Received: {md5_received}")
        return False
    return True


def parse_order_data(raw_order_data):
    """将从API获取的原始订单数据解析并构造成一个干净的字典。"""
    if not raw_order_data:
        raise ValueError("传入的原始订单数据为空。")
        ...
        "line_items": [
            {
                ...
                "option_values": [ov.get("name") for ov in item.get("optionValues", [])]
            } for item in raw_order_data.get("lineItems", [])
        ],
        "total_price": {
            "amount": total_price_obj.get("amount"),
            "currency_code": total_price_obj.get("currencyCode")
        },
        "customer_info": {
...
def process_order_webhook(order_node_id, should_print=True):
    ...
    try:
        ...
        if not db_order_id:
            raise OrderProcessingError("持久化订单失败")
        ...
        if db_order_id:
            # 将错误信息写入订单状态，方便调试
            error_msg = str(e).replace("'", "").replace('"', '')[:50]  # 截断并清理错误信息
...
def handle_webhook():
    """接收、验证并处理来自AllValue的Webhook请求。"""
    if get_setting('polling_enabled') == 'true':
        return jsonify({"status": "ignored", "msg": "Polling is enabled"}), 200
    ...
    if topic and topic.startswith('orders/'):
        ...
        if not order_node_id:
            return jsonify({"status": "fail", "msg": "no nodeId"}), 400
